...
export const command = new Command(
  new SlashCommandBuilder().setName('remove-me').setDescription('S\'ajouter à un événement ou un programme.')
  ...
  async (interaction) => {
    ...
    await eventService.removeEventParticipant(eventId, [interaction.user.id], interaction.user.id);

    const embed = embedUtils.informationEmbed("Participation à un événement", [], "Vous avez bien été retiré de l'événement !")
